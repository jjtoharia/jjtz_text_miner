...
finaliza <- function()
{
    ...
    , "try-error"))
  {
    ...
    if(nErrCount > 5)
    {
      ...
      stop(paste0('ERROR: Demasiados errores intentando escribir en <', local.Proyecto.s, '.CSV>. Detenemos el proceso...'))
    }
    local.Proyecto.s <- paste0(local.Proyecto.s, '.')
    b_CsvExiste <- file.exists(paste0(local.Proyecto.s, '.CSV'))
    print(paste0('WARNING: Probamos a continuar en el fichero <', local.Proyecto.s, '.CSV>...'))
  }
...
# ##################################################
########################################################################################################
Proyecto <- "Prueba tweets 2"
timeout_seconds <- 600 # Keep connection alive for N seconds
trackWords <- c("investidura", "abstención", "PSOE", "Partido Popular", "PP", "Podemos", "Ciudadanos")
########################################################################################################
print(paste('Proyecto =', Proyecto))
print(paste('timeout_seconds =', timeout_seconds))
print(paste('trackWords =', paste0(trackWords, collapse = ',')))

Proyecto.s <- str_replace_all(Proyecto, "\\(|\\)| |:", "_") # Quitamos espacios, paréntesis, etc.

check_Go() # Verificamos fichero GO para salir con Ok o con error 9
...
# ##################################################
# ## Hacemos un bucle de N conexiones de 5 minutos
# ## cada una hasta llegar a timeout_seconds:
# ## NOTA: Se crea un fichero nuevo cada vez
# ##################################################
tryCatch(
  for(i in seq.int(from = 1, to = 10 * timeout_seconds, by = timeout_seconds)) # 600 * 6 * 2 (2 horas)
  {
    nTweets <<- 0
    nSecs <<- 0
    nTweetsDownloaded <<- 0
